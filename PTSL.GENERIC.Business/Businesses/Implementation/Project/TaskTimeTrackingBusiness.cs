...
{
    public class TaskTimeTrackingBusiness : BaseBusiness<TaskTimeTracking>, ITaskTimeTrackingBusiness
    {
        public readonly GENERICUnitOfWork _unitOfWork;
        private readonly GENERICReadOnlyCtx _readOnlyContext;
        public TaskTimeTrackingBusiness(GENERICUnitOfWork unitOfWork, GENERICReadOnlyCtx readOnlyContext)
            : base(unitOfWork)
        {
        ...
        //public override async Task<(ExecutionState executionState, TaskTimeTracking entity, string message)> CreateAsync(TaskTimeTracking entity)
        //{
        //    (ExecutionState executionState, TaskTimeTracking entity, string message) createResponse;

        //    await using (IDbContextTransaction transaction = UoW.Begin())
        //    {
        //        try
        //        {
        //            FilterOptions<TaskTimeTracking> filterOptions = new FilterOptions<TaskTimeTracking>();
        ...
        //            {
        //                createResponse = (executionState: ExecutionState.Success, entity: null, message: $"{typeof(TaskTimeTracking).Name} name already exists with type.");
        //                return createResponse;
        //            }

        //            //Generate Project Request Code By Type
        //            var query = _readOnlyContext.Set<TaskTimeTracking>()
        ...
        //            (ExecutionState executionState, TaskTimeTracking entity, string message) createdResponse = await UoW.CreateAsync<TaskTimeTracking>(entity);
        ...
        public override async Task<(ExecutionState executionState, TaskTimeTracking entity, string message)> UpdateAsync(TaskTimeTracking entity)
        {
            (ExecutionState executionState, TaskTimeTracking entity, string message) updateResponse;
            ...
            await using (IDbContextTransaction transaction = UoW.Begin())
            {
                try
                {
                    ...
                    {
                        updateResponse = (executionState: ExecutionState.Success, entity: null, message: $"{typeof(TaskTimeTracking).Name} name already exists.");
                        return updateResponse;
                    }

                    (ExecutionState executionState, TaskTimeTracking entity, string message) updatedEntity = await UoW.UpdateAsync<TaskTimeTracking>(entity);
                ...
                catch
                {
                    ...
                    updateResponse = (executionState: ExecutionState.Failure, entity: null, message: $"Problem on {typeof(TaskTimeTracking).Name} update.");
                }
            }
        ...
        public async override Task<(ExecutionState executionState, IQueryable<TaskTimeTracking> entity, string message)> List(QueryOptions<TaskTimeTracking> queryOptions = null)
        {
            (ExecutionState executionState, IQueryable<TaskTimeTracking> entity, string message) returnResponse;
            var queryOption = new QueryOptions<TaskTimeTracking>();
            //queryOption.IncludeExpression = x => x.Include(y => y.ProjectRequest!)
            //.Include(y => y.TaskType!)
            //.Include(y => y.User!);
            (ExecutionState executionState, IQueryable<TaskTimeTracking> entity, string message) entityObject = await _unitOfWork.List<TaskTimeTracking>(queryOption);
            returnResponse = entityObject;
