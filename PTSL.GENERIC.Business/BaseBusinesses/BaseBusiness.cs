...
namespace PTSL.GENERIC.Business.BaseBusinesses
{
    public abstract class BaseBusiness<T> : IBaseBusiness<T>, IDisposable where T : BaseEntity
    {
        ...
        public virtual async Task<(ExecutionState executionState, List<T> entity, string message)> UpdateListAsync(List<T> entity)
        {
            (ExecutionState executionState, List<T> entity, string message) updateResponse;
            //(ExecutionState executionState, string message) validateResponse = PreValidation.Updating(entity);
            ...
            await using (IDbContextTransaction transaction = UoW.Begin())
            {
                try
                {
                    ...
                    {
                        (ExecutionState executionState, T entity, string message) updatedEntity = await UoW.UpdateAsync<T>(entityItem);
                        entityList.Add(updatedEntity.entity);
                    }
                    ...
                    if (Guid.TryParse(transaction.TransactionId.ToString(), out Guid transactionGuid))
                    {
                        ...
                        updateResponse = success ? (executionState: saveResponse.executionState, entity: entityList, message: saveResponse.message) : (executionState: saveResponse.executionState, entity: null, message: saveResponse.message);
                    ...
                    {
                        updateResponse = (executionState: ExecutionState.Failure, entity: entityList, message: "Transaction not found.");
                    }
                }
                catch
                {
