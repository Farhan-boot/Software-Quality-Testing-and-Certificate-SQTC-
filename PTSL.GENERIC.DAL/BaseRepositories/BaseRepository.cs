...
namespace PTSL.GENERIC.DAL.Repositories
{
    public abstract class BaseRepository<T> : IBaseRepository<T> where T : BaseEntity
    {
        ...
        public virtual async Task<(ExecutionState executionState, T entity, string message)> UpdateAsync(T entity)
        {
            ...
            if (executionState == ExecutionState.Retrieved && retrievedEntity != null)
            {
                try
                {
                    ...
                    {
                        WriteOnlySet.Add(retrievedEntity).State = EntityState.Modified;
        ...
        public async Task<(ExecutionState executionState, bool isDeleted, string message)> SoftDeleteAsync(long key, long userId)
        {
